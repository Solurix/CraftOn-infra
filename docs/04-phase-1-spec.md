...
3. **Worker onboarding:**
   - Basic info (name, trades, prefecture — prefecture is a picker over the 47
     canonical values, stored as the romaji identifiers used in config/jobs).
   - Work history (職歴): repeatable entries of company / trade / years plus a
     free-text summary (概要) with example placeholder text. Total
     years-of-experience is derived from these entries (no separate input).
